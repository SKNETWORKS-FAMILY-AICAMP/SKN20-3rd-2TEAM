...
def load_json_files(use_weeks: int = 10) -> List[Document]:
    """
    최근 use_weeks주차의 JSON 문서 파일을 로딩

    Args:
        use_weeks: 로딩할 최근 주차 수 (기본값: 10)
...
def save_chunks_to_pkl(
    ...
    Filename Format:
        chunks_{chunk_size}_{chunk_overlap}_C.pkl
    """
    # 저장 폴더 생성 (없으면)
    CHUNKS_DIR.mkdir(parents=True, exist_ok=True)

    # 파일명 생성 (C suffix for clustering)
    filename = f"chunks_{chunk_size}_{chunk_overlap}_C.pkl"
...
def chunk_and_save(
    use_weeks: int = 10, chunk_size: int = 100, chunk_overlap: int = 10
) -> str:
    """
    문서 로딩 → 청킹 → 저장을 한번에 실행
    ...
    사용 예시:
        ...
        # 커스텀 파라미터로 실행
        >>> chunk_and_save(use_weeks=8, chunk_size=200, chunk_overlap=20)
    """
    print("=" * 60)
...
if __name__ == "__main__":
    # 기본 설정으로 청킹 실행
    filepath = chunk_and_save(use_weeks=10, chunk_size=CHUNK_SIZE, chunk_overlap=CHUNK_OVERLAP)
