...
이 서버는 HTML 프론트엔드와 LangGraph RAG 시스템(langgraph_test_2.py)을 연결합니다.
- 통계 정보 제공
- 트렌드 키워드 제공
...
print(f"langgraph_test_2.py 존재: {(RAG_DIR / 'langgraph_test_2.py').exists()}")
print("=" * 70 + "\n")
...
async def startup_event():
    ...
    try:
        # langgraph_test_2 모듈 임포트 시도
        print("\n[STEP 1/2] langgraph_test_2 모듈 임포트 중...")
        
        try:
            from langgraph_test_2 import initialize_langgraph_system, get_system_status
            print("✅ langgraph_test_2 모듈 임포트 성공")
...
async def health_check():
    """헬스 체크"""
    try:
        from langgraph_test_2 import get_system_status
...
async def chat(request: ChatRequest) -> Dict:
    ...
    1. langgraph_test_2.ask_question() 호출
    ...
    try:
        from langgraph_test_2 import ask_question
        ...
        for source in sources_data[:5]:  # 최대 5개
            ...
            if source_type == 'web':
                ...
                formatted_sources.append({
                    "doc_id": str(source.get('source', 'web_unknown'))[:50],
